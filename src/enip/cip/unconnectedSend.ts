...
import { EPathLogical, EPathLogicalTypes } from "./epath/logical";

const UNCONNECTED_SEND_SERVICE = 0x52;
const UNCONNECTED_SEND_PATH = Buffer.concat([
    EPathLogical.build(EPathLogicalTypes.ClassID, 0x06),
    EPathLogical.build(EPathLogicalTypes.InstanceID, 1)
]);
 ...
 * @returns
 */
export function buildUnconnectedSend(message_request: Buffer, path: Buffer, timeout = 5000): Buffer {
