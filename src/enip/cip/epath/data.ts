export enum EPathDataTypes {
    Simple = 0x80,
    ANSI_EXTD = 0x91
};

export enum EPathDataElementTypes {
    UINT8 = 0x28,
    UINT16 = 0x29,
    UINT32 = 0x2a
};

export class EPathData {
     ...
     * @returns
     */
    static build(data: string | Buffer, ANSI = true): Buffer
    {
        // Build Element Segment If Int
        if (typeof data === 'string') return this.elementBuild(parseInt(data));
     ...
     */
    static symbolicBuild(data: string | Buffer, ANSI = true): Buffer {
        // Initialize Buffer
